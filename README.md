...
**130 visible nodes to build with.** [Tessellation](docs/TESSELLATION.md) adds real height displacement, distance detail and optional smoothing. New: layered Fur, Parallax UVs and Parallax Occlusion, shape masks, spirals, rays, brick/honeycomb patterns, triplanar/matcap textures, mesh/view inputs, height/slope/distance masks, and actual triangle wireframes. The pack also includes 40 procedural, coordinate, surface, fur and volume nodes, plus clearer search, typed numeric entry beyond slider ranges, texture pickers, and safer connected-node menus. Fur adds layered shell passes with groom, root/tip color, wind and LOD controls. Parallax changes texture depth and needs tangents; it does not change silhouette. 1D–4D Noise, Musgrave, Voronoi, Checkerboard and Waves; selectable UV channels and coordinate spaces. Seven distortion modes, Gradient, UV Tile/Mirror and Posterize. Same-mesh GPU surface particles; Toon, Unlit, PBR and particle surfaces; particle color, AudioLink, color ramps, rim glow, masked layers, stickers, dissolve, flipbooks, UV distortion, vertex motion and up to eight nested transparent shells. Fold reusable Patterns and preview intermediate values. Fins, baking and measured GPU acceptance remain future work.
