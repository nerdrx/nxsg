...
namespace NXSG.Core
{
    ...
    public static class FeatureNodes
    {
        ...
        {
            ["core.avatarMotion"] = D("Avatar Motion","Inputs","Locomotion inputs supplied by the NXSG FX motion driver: speed and signed sideways/vertical/forward movement in metres per second. No automatic bone or PhysBone tracking.","","speed:float,sideways:float,vertical:float,forward:float,velocity:vector3",false,new JObject()),
            ["core.motionResponse"] = D("Motion Response","Animation","Turn speed into a 0–1 effect amount. Start speed is the dead zone; full speed reaches one. Connect Avatar Motion Speed.","speed:float","value:float",false,new JObject{{"startSpeed",.1},{"fullSpeed",4},{"curve",1}}),
            ["core.motionSway"] = D("Motion Sway","Animation","Normal-direction flutter whose amplitude increases with speed. Connect to surface Displacement. Procedural response, not simulated inertia.","speed:float,mask:float,time:float","value:float",false,new JObject{{"strength",.02},{"frequency",2},{"spatialScale",3},{"fullSpeed",4}}),
            ["core.motionStretchUV"] = D("Motion Stretch UVs","Coordinates","Stretch a UV pattern around its centre as speed increases. Axis 0 is horizontal, 1 vertical. Zero speed preserves UVs.","uv:vector2,speed:float","uv:vector2",false,new JObject{{"strength",.25},{"maxStretch",3},{"axis",0}}),
            ["core.fur"] = D("Fur","Surface","Layered shell fur over an existing surface. Control length and coverage, groom in object space, color roots and tips, and animate wind. More layers add mesh passes.","base:surface,rootColor:color,tipColor:color,length:float,density:float,thickness:float,mask:float,groom:vector3,time:float","surface:surface",false,new JObject{{"fins",0},{"finOpacity",.7},{"layers",16},{"length",.04},{"density",100},{"thickness",.35},{"taper",1},{"gravity",.1},{"windStrength",.1},{"windSpeed",1},{"windScale",2},{"rimStrength",.25},{"lodNear",5},{"lodFar",15},{"minLayers",4},{"receiveShadows",1},{"selfShadowQuality",0},{"selfShadowStrength",1},{"selfShadowBias",.03}}),
