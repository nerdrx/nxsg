    ...
    public sealed partial class GraphWindow
    {
        ...
        void DrawPatternGroups()
        {
            ...
            foreach (var group in GraphGroups.All(graph))
            {
                ...
                var card = new VisualElement { name = "pattern-card", style = { position = UnityEngine.UIElements.Position.Absolute,
                    left = point.x, top = collapsed ? point.y : point.y - 38, width = collapsed ? 240 : 300,
                    backgroundColor = new Color(.12f, .18f, .21f), borderTopLeftRadius = 7, borderTopRightRadius = 7,
