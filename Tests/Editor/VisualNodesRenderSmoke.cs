...
public static class VisualNodesRenderSmoke
{
    static Camera camera; static GameObject quad; static RenderTexture target;
    static readonly string[] Operations={"position","normalDirection","viewDirection","vertexColor","cameraDistance","screenUV","circleMask","boxMask","polygonMask","starMask","radialRays","spiral","brick","hexGrid","triplanarTexture","matcapTexture","rimGlow","heightMask","slopeMask","distanceFade","wireframe"};
    public static void Run()
    {
        try
        {
            ...
            var collage=new Texture2D(7*96,3*96,TextureFormat.RGBA32,false,true);
            for(var index=0;index<Operations.Length;index++)
            {
                ...
                using(var preview=GraphPreview.Create(graph,null))
                {
                    ...
                    if(operation=="triplanarTexture"||operation=="matcapTexture"||operation=="vertexColor")
                        if(center.r<.9f||center.g<.9f)throw new Exception(operation+" white default missing");
                    if(new[]{"circleMask","boxMask","polygonMask","starMask","radialRays","spiral","brick","hexGrid","wireframe"}.Contains(operation))
                    {
                    ...
                    collage.SetPixels(index%7*96,(2-index/7)*96,96,96,pixels);
                }
            }
            collage.Apply();System.IO.Directory.CreateDirectory("Library/NXSG");System.IO.File.WriteAllBytes("Library/NXSG/visual-nodes.png",collage.EncodeToPNG());UnityEngine.Object.DestroyImmediate(collage);
            Debug.Log("NXSG VISUAL NODES RENDER SMOKE PASSED: 21 nodes, masks, texture defaults, finite outputs, distance fade");EditorApplication.Exit(0);
        }
