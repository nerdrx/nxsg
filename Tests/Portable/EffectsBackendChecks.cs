...
public static class EffectsBackendChecks
{
    ...
    {
        foreach(var op in NodeCatalog.All.Where(op => op != "core.parameter" && op != "core.output" && op != "core.shell"))
        {
            ...
            if(type=="surface") { var color=NodeCatalog.Create("core.constant");color.Id="albedo";graph.Nodes.Add(color);Connect(graph,color.Id,"value",node.Id,"albedo");Connect(graph,node.Id,port,"output","surface"); }
            else
            {
                if(type=="vector2")
                {
