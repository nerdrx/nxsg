...
public static class ProceduralChecks
{
    ...
    static void CheckInvalidValues(Action<bool, string> assert)
    {
        ...
        assert(!GraphValidator.Validate(badMusgrave).IsValid, "musgrave ranges reject invalid values");
        var badVoronoi = Graph("core.voronoi", 2);
        badVoronoi.Nodes.Single(n => n.Id == "procedural").Properties["randomness"] = -1;
        assert(!GraphValidator.Validate(badVoronoi).IsValid, "voronoi randomness range rejects");
