...
**Repository added, but no package appears?** Refresh the repository and check that your project uses the supported Unity version. NXSG 1.2.0 is a stable package and does not require prereleases to be enabled.

## Creator Companion

1. Open **Settings → Packages → Add Repository**, paste the URL and confirm the listed repository.
2. Refresh the repository listing. The current stable package is `1.2.0`.
