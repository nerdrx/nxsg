...
| Built-In toon backend, lighting, shadows, stereo, passes, output rollback, and stable assets | D01, D04, D09 | [Platform](research/PLATFORM_AND_INTEGRATIONS.md) and [rendering](research/RENDERING_FUR_AND_PERFORMANCE.md) define a PC Built-In shader target with Linux as the authoring/test environment and separate mobile policy. | S00/S04/S05: Linux Unity 2022.3.22f1, SDK 3.10.5, import/compile, Frame Debugger, local client/VR, mirror, fallback and transaction checks pending. Windows portability is a later check. |
...
1. Can the pinned Linux Unity/VRChat fixture be created and licensed, and what
   local client/headset path is available? Confirm the pins are still accepted;
   no Windows machine is required.
