...
NXSG can run its primary portable graph, compiler, serialization, package, and Unity editor tests on Linux without a Windows machine. Pin Unity **2022.3.22f1** and the VPM dependency graph. VRChat’s official creator guide still gives Windows-first setup guidance and asks creators to check `<DX11>` for the PC target; that is the PC client contract, separate from the Linux editor’s OpenGL/Vulkan renderer. Native Windows checks are later portability evidence. Linux coding and editor package increments can proceed independently; a verified VRChat claim still requires S05 client testing on the actual runtime route. ([VRChat getting started](https://creators.vrchat.com/sdk/), [current Unity version](https://creators.vrchat.com/sdk/upgrade/current-unity-version/))

Linux is the primary authoring and editor-test platform for NXSG. Unity 2022.3 lists Linux OpenGL 3.2+/Vulkan and Windows DX10/DX11/DX12 as distinct renderer paths. Record the active Linux API (OpenGL or Vulkan) in editor evidence; it does not prove native Windows DX11 behavior. The root environment has `/usr/bin/unityhub`, headless-capable `gamescope` 3.16.25, and .NET runtimes 8/9 but no SDK; an Editor was not found in checked Unity paths. A Windows/headset validation environment has not been established. ([Unity 2022.3 system requirements](https://docs.unity3d.com/2022.3/Documentation/Manual/system-requirements.html), [Unity graphics API arguments](https://docs.unity3d.com/2022.3/Documentation/Manual/EditorCommandLineArguments.html))
...
**Recommendation.** Keep the first Linux CI lane `-batchmode -nographics -runTests -testPlatform EditMode`; save XML, Editor log, lock, Unity version, OS, and SHA. Record parsing/emission/import evidence unless explicit shader-compiler output is captured. Add a Linux graphics lane with the active OpenGL/Vulkan API when the pinned Editor is available. Do not hide compiler errors with `-ignorecompilererrors` or auto-mutate the pinned fixture with `-accept-apiupdate`; fail visibly and review API changes.
...
## 2. Authoring renderer and client validation

**Verified.** VRChat instructs creators to switch to `PC, Mac & Linux Standalone` and verify `<DX11>` in the Unity title bar. Unity’s Editor accepts `-force-d3d11` only on Windows; `-force-vulkan` and OpenGL options represent different renderer paths. This makes `<DX11>` a PC client contract, not a Linux authoring prerequisite. Unity’s 2022.3 GPU Usage Profiler documents different Windows and Linux renderer support. ([VRChat SDK setup](https://creators.vrchat.com/sdk/), [Unity command-line graphics options](https://docs.unity3d.com/2022.3/Documentation/Manual/EditorCommandLineArguments.html), [GPU Usage Profiler](https://docs.unity3d.com/2022.3/Documentation/Manual/ProfilerGPU.html))

**Recommendation.** Label evidence records by renderer: `backend-only`, `Linux-OpenGL`, `Linux-Vulkan`, `Windows-DX11`, `Quest/Android-device`, or `iOS-device`. A Linux graphics editor run can prove Linux shader compilation/rendering for its active API; `-nographics` remains parsing/emission/import evidence only. Linux results do not prove native DX11 driver compilation, stereo/mirror behavior in a native Windows client, or upload acceptance. A future Windows smoke can support a native-Windows claim; Proton is optional and must be recorded separately.

**Spike (M).** Create one tiny graph fixture and one representative avatar fixture. First run Unity 2022.3.22f1 on Linux with the active OpenGL/Vulkan API and capture Editor.log, shader compiler output, material preview, and SDK Build & Test evidence where available. If a Proton path is later established, record it as a separate runtime route; only a native Windows DX11 run supports a native-Windows claim. Compare generated ShaderLab and diagnostics, never screenshots as interchangeable platform proof.
...
**Recommendation.** Support Linux as NXSG’s primary authoring, Unity-editor, compiler, package, and test platform. Preserve the official VCC statement that Windows is its fully supported GUI context and Linux CLI paths are untested; treat that as support context, not a Windows prerequisite for Linux milestones or package release. Report the active Linux renderer and distinguish Linux editor evidence from native Windows client evidence. Use a future native-Windows smoke or an explicitly recorded Proton route only when making those portability/runtime claims.

Observed local minimum: Unity Hub exists, but an Editor was not found in checked `$HOME/Unity` or `$HOME/UnityHub` locations; `dotnet --list-sdks` is empty; Linux editor execution and active renderer are not yet established in this workspace. Headless Gamescope is an execution primitive, not Unity/VRChat proof. Windows is not a blocker for Linux coding or package milestones.
...
| P0 | Clean Linux Unity 2022.3.22f1 EditMode project; `-batchmode -nographics` | Import, domain reload, compiler, graph asset, rollback and material-preservation tests pass | Unity log, `packages-lock.json`, XML results, OS/Editor version |
| P1 | Linux Unity 2022.3.22f1 graphics editor, active OpenGL/Vulkan | Fixture reaches explicit shader compile and material preview without errors | Active API, Editor log, shader compiler output, screenshots |
| P1 | Linux VRChat SDK avatar Build & Test route, if available | Avatar loads, animation properties work, fallback and validation messages are correct | SDK version, actual Linux client route and runtime version, target, build report, test result |
...
Automated backend and Linux Editor tests can establish deterministic generation, package integrity, rollback, and Linux renderer behavior when the graphics lane runs. They cannot establish native Windows DX11 behavior, native-client VR stereo/mirror parity, Quest/iOS policy acceptance, or upload success. Those require live records from the claimed runtime route and target device; Proton evidence must not be presented as native Windows evidence.

## Practical blockers and source ledger

The immediate blockers are that an Editor was not found in the checked paths, the .NET SDK is missing for the official VPM CLI, and Linux editor execution/active renderer are not yet established. No installation or activation was performed during the research phase. Windows is a future portability/runtime claim gate, not a prerequisite for Linux coding, editor milestones, or package releases. Local checks can still run source-level audits, package-zip audits, and deterministic compiler tests once implementation exists.

| Source | Fact used | Checked |
|---|---|---|
| VRChat Creator Docs | Windows-first setup context, `<DX11>` PC client contract, Unity 2022.3.22f1 | 2026-09-17 |
...
| Local observation | `/usr/bin/unityhub`; no checked Editor; .NET runtimes 8/9 but no SDK; headless Gamescope available; Linux renderer not tested | 2026-09-17 |
